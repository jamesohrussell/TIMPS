...
class C3DFrameObject
	...
	// overlaps in previous timestep
	//vector <double> overlaps;
	COverlap_list overlap_list;
	//COverlap_list overlap_list_new_id;
	...
	/*bool compare_are_they_the_same(const C3DFrameObject &f) const
		{
		...
		if (zmin != f.zmin) return(false);
		if (zmax != f.zmax) return(false);
	...
	bool operator== (const C3DFrameObject& f) const
	...
	/*void add_overlap(double id)
		{
 		// check if allready in the array
 		if (!binary_search(overlaps.begin(), overlaps.end(),id))
 			{
			// add the id
...
class C3DFrameObjects_all
	...
	/*void set_new_ids_from_overlaps_using_Forward_in_Time_logic()
		{
		long it,io;
		...
		for (it=0; it < (long)FrameObjects_in_timestep.size(); it++)
			for (io=0; io < (long)FrameObjects_in_timestep[it].FrameObject.size(); io++)
				if (FrameObjects_in_timestep[it].FrameObject[io].object_id > max_id)
					max_id=FrameObjects_in_timestep[it].FrameObject[io].object_id;

		double new_id_counter=100000;
		if (max_id > new_id_counter)
			error(AT,FU, "max_id > start_of_new_id");
		...
		for (it=0; it < (long)FrameObjects_in_timestep.size(); it++)
			for (io=0; io < (long)FrameObjects_in_timestep[it].FrameObject.size(); io++)
				...
				// dont do anything for zero sized objects - this are objects with id-s up to 10
				if (fo->size > 0)
					{
					// no overlap - new object
					if (fo->overlap_list.overlap.size() == 0)
					...
					// one or more overlaps - new_id is the same as new_id of overlaping object in previous time step which has the biggest overlapping size
						...
						fo2 = &FrameObjects_in_timestep[it-1].FrameObject[overlap.id];
						fo->new_id=	fo2->new_id;
						}
					}
				}
		...
		for (it=0; it < (long)FrameObjects_in_timestep.size(); it++)
			for (io=0; io < (long)FrameObjects_in_timestep[it].FrameObject.size(); io++)
				if (FrameObjects_in_timestep[it].FrameObject[io].size > 0)
					...
					s1 << fo->custom_output_for_table2_for_project_of_comparison_of_identification_algorithms_HDCP2(it);
					}
