...
datadir = "/uufs/chpc.utah.edu/common/home/varble-group1/ERA5/moisture/TCWV/"
fileidin = "ERA5.TCWV."
datadirout = "/uufs/chpc.utah.edu/common/home/u0816744/zips2/ERA5_derived/moisture/TCWV/"
fileidout1 = "ERA5.TCWV.anomaly_"
...
def anomaly(d):
  ...
  la = la[ilaN:ilaS+1]
  var0 = f0.variables["TCWV"][:,ilaN:ilaS+1,:]
  f1 = nc.Dataset(dirfiles[1])
  var1 = f1.variables["TCWV"][:,ilaN:ilaS+1,:]
  ti = f1.variables["time"][:]
  f2 = nc.Dataset(dirfiles[2])
  var2 = f2.variables["TCWV"][:,ilaN:ilaS+1,:]
  ...
  mfns.write_var("time","Time","","time",
   np.float64,"hours since 1900-01-01 00:00:00",fileout1,ti)
  ...
  mfns.write_var_compress("TCWV",
   "Total column water vapor","Full field",
   ("time","latitude","longitude"),np.float64,"kg m**-2",
   fileout1,var1,cl=1,lsd=2)
  ...
  mfns.write_var_compress("TCWV_anom",
   "Total column water vapor anomaly",
   "Anomaly from 30 day running mean",
   ("time","latitude","longitude"),
   np.float64,"kg m**-2",fileout1,varanom,cl=1,lsd=2)

  mfns.write_var_compress("TCWV_anomn",
   "Normalized total column water vapor anomaly ",
   "Anomaly from 30 day running mean normalized by 30 day running standard deviation ([var - var_mean]/var_std)",
   ("time","latitude","longitude"),np.float64,"",
   fileout1,varanompct,cl=1,lsd=2)
