...
datadir = "/uufs/chpc.utah.edu/common/home/zipser-group2/ERA5_derived/moisture/SPHU/"
fileidin = "ERA5.SPHU_1000-850hPamean."
datadirout = "/uufs/chpc.utah.edu/common/home/u0816744/zips2/ERA5_derived/moisture/SPHU/"
fileidout1 = "ERA5.SPHU_1000-850hPamean.anomaly_"
...
def anomaly(d):
  ...
  la = la[ilaN:ilaS+1]
  var0 = f0.variables["Q"][:,ilaN:ilaS+1,:]
  f1 = nc.Dataset(dirfiles[1])
  var1 = f1.variables["Q"][:,ilaN:ilaS+1,:]
  ti = f1.variables["time"][:]
  f2 = nc.Dataset(dirfiles[2])
  var2 = f2.variables["Q"][:,ilaN:ilaS+1,:]
  ...
  mfns.write_var("time","Time","","time",np.float64,
   "hours since 1900-01-01 00:00:00",fileout1,ti)

  mfns.write_var("latitude","Latitude","","latitude",
   np.float64,"degreesN",fileout1,la)

  mfns.write_var("longitude","Longitude","","longitude",
    np.float64,"degreesE",fileout1,lo)

  # Create variables in file
  mfns.write_var_compress("Q",
   "Specific humidity average over 1000-850 hPa","Full field",
  ...
  mfns.write_var_compress("Q_anom",
   "Specific humidity anomaly average over 1000-850 hPa",
   "Anomaly from 30 day running mean",
   ("time","latitude","longitude"),
   np.float64,"kg kg**-1",fileout1,varanom,lsd=5)

  mfns.write_var_compress("Q_anomn",
   "Normalized specific humidity anomaly average over 1000-850 hPa",
